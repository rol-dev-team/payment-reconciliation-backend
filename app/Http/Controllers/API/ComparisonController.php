...
class ComparisonController extends Controller
{
    ...
    public function update(Request $request, Comparison $comparison): JsonResponse
    {
        $request->validate([
            ...
            'channel_id'        => 'nullable|exists:payment_channels,id',
            'wallet_id'         => 'nullable|exists:wallets,id',
            'billing_system_id' => 'nullable|exists:billing_systems,id',
        ]);
 
        DB::beginTransaction();
 
        try {
            // 1️⃣ Snapshot BEFORE state into history
            ComparisonHistory::create([
                ...
                'sender_no'         => $comparison->sender_no,
                'trx_date'          => $comparison->trx_date,
                'vendor_trx_date'   => $comparison->getOriginal('vendor_trx_date'),
                'billing_trx_date'  => $comparison->getOriginal('billing_trx_date'),
                'entity'            => $comparison->entity,
                'customer_id'       => $comparison->customer_id,
                'amount'            => $comparison->amount,
            ...
            $comparison->update([
                ...
                'entity'            => $request->input('entity',            $comparison->entity),
                'amount'            => $request->input('amount',            $comparison->amount),
                'vendor_trx_date'   => $request->input('vendor_trx_date',   $comparison->vendor_trx_date),
                'billing_trx_date'  => $request->input('billing_trx_date',  $comparison->billing_trx_date),
                'status'            => $request->input('status',            $comparison->status),
                'is_billing_system' => $request->input('own_db',            $comparison->is_billing_system),
                'is_vendor'         => $request->input('is_vendor',         $comparison->is_vendor),
                'channel_id'        => $request->input('channel_id',        $comparison->channel_id),
                'wallet_id'         => $request->input('wallet_id',         $comparison->wallet_id),
                'billing_system_id' => $request->input('billing_system_id', $comparison->billing_system_id),
            ]);
 
            // 3️⃣ Snapshot AFTER state into history
            $fresh = $comparison->fresh();
            ComparisonHistory::create([
                'comparison_id'     => $fresh->id,
                'snapshot_type'     => 'after',
                'batch_id'          => $fresh->batch_id,
                'process_no'        => $fresh->process_no,
                'trx_id'            => $fresh->trx_id,
                'billing_system_id' => $fresh->billing_system_id,
                'sender_no'         => $fresh->sender_no,
                'trx_date'          => $fresh->trx_date,
                'vendor_trx_date'   => $fresh->vendor_trx_date,
                'billing_trx_date'  => $fresh->billing_trx_date,
                'entity'            => $fresh->entity,
                ...
                'status'            => $fresh->status,
                'is_vendor'         => $fresh->is_vendor,
                'is_billing_system' => $fresh->is_billing_system,
            ]);
            ...
            return response()->json([
                'success' => true,
                'data'    => $fresh->load(['billingSystem', 'channel', 'wallet']),
            ]);
